...
\author{Rohith Madhani\\EE23BTECH11038}
\date{}
...
\begin{align}
    ...
    j &= 12
\end{align}
 substituting $j = 12$ gives the passband
range for our bandpass filter as $11.2$ kHz - $12.4$ kHz.  Hence, the un-normalized discrete time filter
passband frequencies are $F_{p1} = 11.2$ kHz and $F_{p2} = 12.4$ kHz. \\
The corresponding normalized digital filter passband frequencies are
\begin{align}
    \omega_{p1} &= 2\pi\frac{F_{p1}}{F_s} = 0.466 \pi\\
    \omega_{p2} &= 2\pi\frac{F_{p2}}{F_s} = 0.516 \pi
\end{align}
...
\begin{align}
    F_{s1} &= 11.2 - 0.3 = 10.9 \text{kHz}\\
    F_{s2} &= 12.4 + 0.3 = 12.7 \text{kHz}
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.454 \pi\\
    \omega_{s2} &= 2\pi\frac{F_{s2}}{F_s} = 0.529 \pi
\end{align}
\end{enumerate}
...
$\Omega_{p1} = 0.8985$, $\Omega_{p2} = 1.0515$ and $\Omega_{s1} = 0.865$, $\Omega_{s2} = 1.0955$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.9719$ and $B = \Omega_{p2} - \Omega_{p1} = 0.153$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.483\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.524
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.483.
\end{align}
...
we get $N\geq 4$ and $0.298 \leq \epsilon \leq 0.61$\\
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/RohithMadhani/EE1205/tree/main/Filter_Design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/plot1.png}
\caption{The Analog Low-Pass Frequency Response for $0.298  \leq \epsilon \leq 0.61$}
...
\begin{lstlisting}
https://github.com/RohithMadhani/EE1205/blob/main/Filter_Design/codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/RohithMadhani/EE1205/blob/main/Filter_Design/codes/pole_zero_plt.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
     ...
     &= \frac{0.3125}{{s_L}^4 + 1.1068{s_L}^3 + 1.61245{s_L}^2 + 0.91397s_L + 0.33656}\label{eq:design}
\end{align}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{1.1712 \times 10^{-5} s^4}{s^8 + 0.169s^7 + 3.816s^6 + 0.4831s^5 + 5.4249s^4 + 0.4563s^3 + 3.404s^2 + 0.143s + 0.796} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{align}
    H_{d,BP}(z) &= \frac{G\brak{1 - 4z^{-2} + 6z^{-4} - 4z^{-6} + z^{-8}}}{15.6923 - 3.4896z^{-1} + 59.957z^{-2} - 9.979z^{-3} + 86.069z^{-4} - 9.573z^{-5} + 54.979z^{-6} - 3.07z^{-7} + 13.189z^{-8}}
\end{align}

where $G=1.1712 \times 10^{-5}$    
\begin{figure}[H]
\centering
...
\begin{align}
     \frac{\sin(\omega_{p2} n)}{n\pi} -\frac{\sin\brak{\omega_{p1}n}}{n\pi} &= 2\cos{\brak{\frac{\omega_{p2}n+\omega_{p1}n}{2}}}\sin{\brak{\frac{\omega_{p2}n-\omega_{p1}n}{2}}}\\
            &= \frac{2\cos{\brak{0.491n\pi}}\sin{\brak{0.025n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{0.491n\pi}}\sin{\brak{0.025n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
\end{cases} \label{eq:h(n)desired_for_LPF}
\end{align}
